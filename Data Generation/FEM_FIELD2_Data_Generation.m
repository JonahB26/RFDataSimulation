...
% dataResultsFolder = 'D:/dataResults_FEA_FIELDII';
% 
% % Here we'll create a folder and directory for the files
% DataFolder = 'D:/OutputYMs';
...
%for i = 1:length(Files)
    ...
    axial_displacements = repmat(linspace(-.5,0,axRes)',1,latRes);
   % lateral_displacements = repmat(linspace(-2.0,0,axRes)',1,latRes);
...
I = Output.Image.loaded/max(Output.Image.loaded(:));% Need to change this so that it's good
% image for scatterers. Also need to normalize it.
...
imageopts = ImageOpts((transducer.N_elements-transducer.N_active)/2, (transducer.element_width+transducer.kerf)*(transducer.N_elements-transducer.N_active),...
    40/1000, 40/1000,10/1000, 114000,100);
...
imshow(imresize(BMODE1, [axRes-1,latRes-1]),[])
title('Frame 1')
colorbar
...
FIELD2Results.transducer.transducer_used = transducernum;
...
%save(filename,"FIELD2Results",'-append')

% IF STILL GETTING ISSUES, CHECK WITH FOCUS IN TRANSDUCER
% checl on that error warning about aperture!
