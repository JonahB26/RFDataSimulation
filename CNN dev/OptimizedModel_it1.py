...
def load_data(images, labels, batch_size=16, train_split=0.7):
...
def objective(trial):
    ...
    try:
        ...
        labels = np.load('images_prePCA.npy')
        train_loader, val_loader = load_data(images, labels, batch_size)

        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        model = OptimizedCNN(num_filters).to(device)
        ...
        history = train_with_gradient_accumulation(
            ...
            optimizer, 
            device, 
            num_epochs=5,  # Reduced epochs for trials
...
def main():
    ...
    if not os.path.exists('images_prePCA.npy') or not os.path.exists('images_prePCA.npy'):
        process_data()
